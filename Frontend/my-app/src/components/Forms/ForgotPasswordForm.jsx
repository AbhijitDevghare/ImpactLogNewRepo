...
import { verifyEmail, sendOtp, setForgotPasswordData } from "../../redux/slices/AuthSlice";
...
const ForgotPasswordForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEmail) {
        await dispatch(verifyEmail(input.trim())).unwrap();
        dispatch(setForgotPasswordData({ email: input.trim() }));
        toast.success("Verification email sent!");
      } else {
        await dispatch(sendOtp({ phoneNumber: input.trim(), email: "" })).unwrap();
        dispatch(setForgotPasswordData({ phoneNumber: input.trim() }));
        ...
        <div className="rounded-[28px] p-[1.5px] bg-gradient-to-b from-gray-700/70 to-gray-700/20 shadow-[0_12px_40px_rgba(0,0,0,0.25)]">
          <div className="rounded-[26px] bg-gray-800/80 backdrop-blur-xl border border-gray-700/60 px-8 sm:px-10 py-10">
            ...
            <form className="flex flex-col gap-5" onSubmit={handleSubmit}>
              <div className="flex flex-col gap-2">
                ...
                <input
                  id="input"
                  type="text"
                  placeholder="yourname@impactlog.com or +1234567890"
                  value={input}
            ...
            <div className="text-center mt-5">
              <button onClick={() => navigate("/login")} className="text-sm text-purple-400 hover:underline">
                Back to Login
              </button>
            </div>
          </div>
